...
export_element(E = #xmlElement{name = Tag,
			       ...
			       parents = Parents,
			       content = Content}, CBs) ->
%    io:format("~p:export_element E=~p~n",[?MODULE,E]),
    Data = export_content(Content, CBs),
    Args = [Data, Attributes, Parents, E],
    tagdef(Tag,Pos,Parents,Args,CBs);
export_element(E=#xmlPI{}, _CBs) ->
    io:format("~p:export_element E=~p~n",[?MODULE,E]),
%    tagdef(Tag,Pos,Parents,Args,CBs);
    [];
export_element(E=#xmlComment{pos = Pos,
			     parents = Parents,
			     value = Value}, CBs) ->
%    io:format("~p:export_element E=~p~n",[?MODULE,E]),
    Args = [Value, [], Parents, E],
    tagdef(xmlComment,Pos,Parents,Args,CBs);
export_element(E=#xmlDecl{attributes = Attributes}, CBs) ->
    Data=[],
    Parents=undefined,
    Pos=1,
    Args = [Data, Attributes, Parents, E],
    tagdef(xml,Pos,Parents,Args,CBs).
