...
'#root#'(Data, [#xmlAttribute{name=prolog,value=V}], [], _E) ->
    [V,Data];
'#root#'(Data, _Attrs, [], _E) ->
    %% XML declaration should have been exported as an element (#xmlDecl{})
    [Data,?lf].


%% The '#element#' function is the default handler for XML elements.

'#element#'(_, Value, _Attrs, _Parents, E) when is_record(E,xmlComment) ->
    ["<!--",Value,"-->",?lf];
'#element#'(_, [], Attrs, _Parents, E) when is_record(E,xmlDecl) ->
    ["<?xml",attributes(Attrs),"?>",?lf];
